...
export function checkExistKey(config: ProviderSettings | undefined) {
	...
	// Special case for fake-ai, claude-code, qwen-code, roo, and gemini-cli providers which don't need any configuration.
	if (
		config.apiProvider &&
		["fake-ai", "claude-code", "qwen-code", "roo", "gemini-cli"].includes(config.apiProvider)
	) {
		return true
	}
